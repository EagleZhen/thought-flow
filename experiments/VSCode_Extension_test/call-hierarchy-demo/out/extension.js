...
async function getCallHierarchy(document, position) {
    try {
        ...
        const hierarchyItems = await vscode.commands.executeCommand('vscode.prepareCallHierarchy', document.uri, position);
        ...
        const incomingCalls = await vscode.commands.executeCommand('vscode.provideIncomingCalls', targetSymbol);
        ...
        const outgoingCalls = await vscode.commands.executeCommand('vscode.provideOutgoingCalls', targetSymbol);
...
const customProvider = {
    ...
    async prepareCallHierarchy(document, position) {
        ...
        const symbolItem = new vscode.CallHierarchyItem(vscode.SymbolKind.Function, symbolName, '', document.uri, nameRange, // fullRange - the entire function
    ...
    async provideCallHierarchyIncomingCalls(item) {
        ...
        for (const fileUri of pythonFiles) {
            ...
            while ((regexMatch = functionCallRegex.exec(fileContent)) !== null) {
                ...
                const callerItem = new vscode.CallHierarchyItem(vscode.SymbolKind.Function, document.getText(callRange), '', fileUri, callRange, callRange);
    ...
    async provideCallHierarchyOutgoingCalls(item) {
        ...
        while ((regexMatch = functionCallPattern.exec(functionBodyText)) !== null) {
            ...
            const calleeItem = new vscode.CallHierarchyItem(vscode.SymbolKind.Function, calledFunctionName, '', item.uri, callRange, callRange);
...
function activate(context) {
    ...
    const commandDisposable = vscode.commands.registerCommand('callHierarchyDemo.showCallHierarchy', async () => {
        try {
            ...
            const toRelativePath = (uri) => vscode.workspace.asRelativePath(uri, false);
            ...
            if (workspaceFolders && workspaceFolders.length > 0) {
                try {
                    ...
                    await vscode.workspace.fs.writeFile(outputFileUri, Buffer.from(JSON.stringify(callHierarchyData, null, 2)));
